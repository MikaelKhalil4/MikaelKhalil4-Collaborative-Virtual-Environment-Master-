...
namespace RVC {
    ...
    public class Grabable : MonoBehaviourPun {
        ...
        public virtual void LocalRelease () {
    		...
    		if (PhotonNetwork.IsConnected) {
       		    // add code here
                photonView.RPC ("Release", RpcTarget.Others) ; // same for "Release", "ShowCatchable", "HideCatchable"
            }
            Release () ;
        }
        ...
        public void LocalShowCatchable () {
            ...
            if (! caught) {
                ...
                if (PhotonNetwork.IsConnected) {
           	        // add code here
                    photonView.RPC ("ShowCatchable", RpcTarget.Others) ; // same for "Release", "ShowCatchable", "HideCatchable"
                }
            } else {
        ...
        public void LocalHideCatchable () {
            ...
            if (! caught) {
                ...
                if (PhotonNetwork.IsConnected) {
           		    // add code here
                    photonView.RPC ("HideCatchable", RpcTarget.Others) ; // same for "Release", "ShowCatchable", "HideCatchable"
                }
            } else {
                numberOfTools = numberOfTools - 1 ;
            }
